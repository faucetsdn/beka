...
class IPAddressTestCase(unittest.TestCase):
    ...
    def test_from_string(self):
        ipv4_address_strings = ["10.1.1.1", "10.1.0.0", "192.168.0.23"]

        ipv6_address_strings = ["2404:138::1", "2404:138::", "2404:138::234:1", "::1"]
        ...
        for address, address_string in zip(ipv4_addresses, ipv4_address_strings):
            ...
            self.assertEqual(
                repr(address), 'IP4Address.from_string("%s")' % address_string
            )
        for address, address_string in zip(ipv6_addresses, ipv6_address_strings):
            self.assertEqual(str(address), address_string)
            self.assertEqual(
                repr(address), 'IP6Address.from_string("%s")' % address_string
            )
...
class IPPrefixTestCase(unittest.TestCase):
    ...
    def test_from_string(self):
        ipv4_prefix_strings = ["10.1.1.1/32", "10.1.0.0/16", "192.168.0.128/20"]
        ...
        for prefix, prefix_string in zip(ipv4_prefixes, ipv4_prefix_strings):
            ...
            self.assertEqual(
                repr(prefix), 'IP4Prefix.from_string("%s")' % prefix_string
            )
        for prefix, prefix_string in zip(ipv6_prefixes, ipv6_prefix_strings):
            self.assertEqual(str(prefix), prefix_string)
            self.assertEqual(
                repr(prefix), 'IP6Prefix.from_string("%s")' % prefix_string
            )
